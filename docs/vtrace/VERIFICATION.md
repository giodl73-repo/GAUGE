...
requirements. GAUGE is greenfield, so implementation-dependent checks are
`pending` (no code/corpus to run yet). Process-level requirements that already have
evidence (VTRACE discipline, child-repo scoping, the review panel) are verified
now. A method being named here does not mean it has run — `EVID-*` records only
actual results.
...
|---|---|---|---|---|---|
| REQ-001 | inspection / demonstration | inspect corpus regeneration commands once built | a documented regeneration path with labels preserved | pending | future EVID-001 |
...
| REQ-004 | schema check / inspection | validate corpus frontmatter keys against schema | stable station/segment/corridor id present; labels are not keys | pending | future EVID-004 |
| REQ-005 | gate / data inspection | hold/reject check on unidentified or uncited rows | unidentified/uncited rows held, not promoted | pending | future EVID-005 |
...
| REQ-007 | analysis / inspection | inspect reliability/frequency claims for named dispatch basis | dedicated vs shared-host basis named on each claim | pending | future EVID-007 |
| REQ-008 | gap inspection / review | inspect a car/air-dominant market's gap artifact | null result recorded, no forced rail case | pending | future EVID-008 |
...
| REQ-014 | schema check / inspection | validate `tier` + `sla` frontmatter and tier model | every corridor classified T1–T4 with declared SLA | pending | future EVID-014 |
...
# Implementation levels (planned — once gauge-network/CLI exist)
# cargo fmt --check
# cargo test
# gauge corpus regenerate ; gauge tier-sla --gate
```
...
| L2 | Readiness proof before a public claim. | corpus regeneration + `tier-sla --gate` + role review | pending (greenfield) |
...
| EVID-001..008, 014, 015 | (pending) | implementation-dependent | REQ-001..008/014/015 | pending |
